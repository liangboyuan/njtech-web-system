...
class Conf extends Common
{
    ...
    public function add()
    {
        ...
        if(request()->isPost()){
            $data = input('post.');
            if($data){
                $res=db('catename')->where('id',1)->update($data);
                if($res){
                    $this->success('更新数据成功','lst');
                }else{
                    $this->error('更新数据失败','lst');
                }
            }
        }
        return view();
    ...
    public function conf(){
        if(request()->isPost()){
            $data=input('post.');
            if($data){
                $insert=db('them')->insert($data);
                if($insert){
                    $this->success('主题名称添加成功','lst');
                }else{
                    $this->error('主题添加失败','lst');
                }
            }
        }
        return view();
    ...
    public function del($id){
        $data=db('them')->where('id',$id)->delete();
        if($data){
            $this->success('专题栏目删除成功','lst');
        }else{
            $this->error('专题栏目删除失败','lst');
        }
    }

    public function edit($id){
        $data=db('them')->where('id',$id)->find();

        if(request()->isPost()){
            $data1=input('post.');
            $conf=new ConfModel();
            $res=db('them')->update($data1);
            if($res){
